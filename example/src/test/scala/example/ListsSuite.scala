 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of a few numbers") {
    ...
    assert(sum(List(2,12,-3)) === 11)
    assert(sum(List(6,7,-17)) === -4)
  }
  ...
  test("sum of repeated elements") {
    assert(sum(List(1,1,1,2,2,3)) === 10)
  }
  ...
  test("max of repeated elements") {
    assert(max(List(1,1,1,2,2)) === 2)
  }

  test("max of zero elements") {
  ...
  test("max of positive and negative elements") {
    assert(max(List(0, 5,-6)) === 5)
  }

  test("max throws NoSuchElementException if its argument is empty list") {
